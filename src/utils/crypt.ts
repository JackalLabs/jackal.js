...
import { hashAndHex, hexToBuffer } from '@/utils/hash'
...
export async function convertToPublicFile (workingFile: File): Promise<File> {
  const chunkSize = 32 * Math.pow(1024, 2)
  /** in bytes */
  const details = {
...
export async function compressEncryptString (
  ...
  let compString = compressData(input)

  const data = await cryptString(compString, key, iv, 'encrypt', isLedger)
  console.log(data)
  return data
}
...
export async function decryptDecompressString (
  ...
  console.log('decryptDecompressString 1', input)

  if (input.startsWith('jklpc2|')) {
    const wasBase64 = atob(input.substring(7))
    const moar = [...wasBase64].map((str) => str.codePointAt(0) || 0)
    const safe = Uint8Array.from(moar)

    // const safe = hexToBuffer(input.substring(7))
    console.log('decryptDecompressString safe len', safe.byteLength)
    // const uint = uint8StringToUint16(uint8ToString(safe))
    // console.log('decryptDecompressString uint len', uint.byteLength)
    // input = uint16ToString(uint)

    input = uint16ToString(new Uint16Array(safe.buffer))
  }
  console.log('decryptDecompressString 2', input)
